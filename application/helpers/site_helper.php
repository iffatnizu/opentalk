...
function tokenDetails($token) {
    ...
    return $CI->model_home->getTokenDetails($token);
}

function getTimeZone() {
    $CI = &get_instance();
    $CI->load->model('model_home');
    return $CI->model_home->getTimeZone();
}
...
function getSiteTime($timestamp = "") {
    ...
    $dtzone = new DateTimeZone($selectedzone);
...
function setAppointmetTime($timestamp) {

    return strtotime($timestamp);
    $selectedzonearr = getTimeZone();
    $userTimezone = new DateTimeZone($selectedzonearr[DBConfig::TABLE_SETTING_ATT_TIMEZONE]);
    $gmtTimezone = new DateTimeZone('GMT');
    $myDateTime = new DateTime($timestamp, $gmtTimezone);
    $offset = $userTimezone->getOffset($myDateTime);
    //echo $offset . '<br/>';

    return strtotime(date('m-d-Y, \a\\t g.i A', $myDateTime->format('U') - $offset));
}

function getSliderTime($timestamp = "") {

    $CI = & get_instance();
    $CI->load->helper('date');
    $datestring = "%Y:%m:%d:%H:%i";
    $time = time();
    $temp = mdate($datestring, $time);
    //echo $temp;
    return explode(":", $temp);
...
function getOpentokSetting(){
    $CI = &get_instance();
    $CI->load->model('model_home');
    return $CI->model_home->getOpentokSetting();
}